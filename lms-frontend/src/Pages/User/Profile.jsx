...
function Profile() {
    ...
    const [isCancelling, setIsCancelling] = useState(false); // Optional: Loading state

    async function handleCancellation() {
        setIsCancelling(true); // Optional
        toast("Initiating cancellation");
        await dispatch(cancelCourseBundle());
        await dispatch(getUserData());
        toast.success("Cancellation completed!");
        setIsCancelling(false); // Optional
        navigate("/");
    }
                ...
                <div className="my-10 flex flex-col gap-4 rounded-lg p-4 text-white w-96 shadow-[0_0_10px_black]">
                    ...
                    {userData?.subscription?.status === "active" && (
                        <button
                            ...
                            disabled={isCancelling} // Optional
                            className="w-full bg-red-600 hover:bg-red-500 transition-all ease-in-out duration-300 rounded-sm font-semibold py-2 cursor-pointer disabled:opacity-50"
                        >
                            {isCancelling ? "Cancelling..." : "Cancel Subscription"} {/* Optional */}
                        </button>
                    )}
                </div>
            </div>
